...
void loop() {
  ...
  adjusted_voltage = constrain((6*(map(input_voltage, 0, 1023, 0, 255))),0,255);     
  // We want to re-map the input voltage value to the range [0, 255] 
  // We multiply by 6 to account for the low input voltage     
  ...
  delay(1000); // One second delay, so serial output and LED are only updated once per second
...
/*
        We first set which pin the ADC will read from using the ADMUX register.Bits 3:0 set which pin, and A0 is set by 0000.
